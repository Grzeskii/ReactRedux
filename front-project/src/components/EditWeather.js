...
import { weatherEdit } from '../actions/actions'
...
const EditWeather = ({ reduxState, weatherEdit }) => {
    ...
    return (
        <>
        <Navbar />
            <div className="weather-edit" style={{display: "flex", justifyContent: "center", font: "Roboto"}}>
                <Formik 
                    initialValues={{
                        "name": detailsElement.name,
                        "weather_state": detailsElement.weather_state,
                        "min_temp": detailsElement.min_temp,
                        "max_temp": detailsElement.max_temp,
                        "meas_date": detailsElement.meas_date,
                        "comments": detailsElement.comments,
                    }}
                    validate={(values) => {
                            const errors = {}
                    ...
                    onSubmit={(data, { resetForm }) => {
                        weatherEdit(id, data.name, data.weather_state, data.min_temp, data.max_temp, data.meas_date, data.comments)
                        resetForm()
                        history.push('/table')
                    }}
                    >
...
const mapDisptachToProps = disptach => {
    return {
        weatherEdit: (id, cityName, ws, min, max, dt, cmt) => disptach(weatherEdit(id, cityName, ws, min, max, dt, cmt))
    }
}

export default connect(mapStateToProps, mapDisptachToProps)(EditWeather)